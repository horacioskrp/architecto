...
`{type:"error"}`. On failure, the detail (traces, provider message, paths) is
**logged server-side**; the client only receives a generic message, with no
information leak.
...
(per-project ADRs). Conversations are **durably persisted**: a JSON file in the
*main process* (`userData` folder, via the `window.api.conversations` IPC bridge),
with a `localStorage` **fallback** for the dev browser and automatic migration of
any legacy history. Writes are **debounced**: during streaming the content mutates
on every token, so it is written after an idle window. Loading happens via
`ChatStore.hydrate()` (async).
